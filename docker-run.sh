...
if [ "$(id -u)" = 0 ]; then
	...
	if find *-cookie.json > /dev/null 2>&1; then
		# make sure cookie files are writeable
		chown node:node *-cookie.json
	fi
...
exec $su_exec /usr/local/bin/node '/opt/mx-puppet-linkedin/dist/index.js' \
     -c "$CONFIG_PATH" \
     -f "$REGISTRATION_PATH" \
     $args
